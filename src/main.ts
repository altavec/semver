...
async function run(): Promise<void> {
  try {
      // install semver
      let installArgs = ['tool', 'install', '-g', 'mondo.semanticversioning.teamcity'];
      ...
      let versionJson = '';
      await exec('dotnet-semver', args, { listeners: { stdout: (data: Buffer) => { versionJson += data.toString() } } });
      ...
      for (let name in versionProperties) {
